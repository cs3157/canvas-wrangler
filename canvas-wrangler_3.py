...
# replace with your own Canvas authentication key
AUTHVAR = 'CANVASPONIES'

# default columns to search for in csv
STUDENT_COL = 'uni'
GRADE_COL = 'grade'
COMMENT_COL = 'comment'
...
if __name__=='__main__':
    ...
    if not AUTHVAR in os.environ.keys():
        print('Error: authentication token enviromental variable',
              AUTHVAR, 'not set.')
        exit(1)
    ...
    # populate {UNI: user id} dict from sdb.csv
    students = {}
    for s in sdb:
        students[s[0]] = s[1]

    header_col = next(grades)
    ...
    if submit_comment:
        ...
        try:
            ...
            print('Error: could not find comment column header:',
                  args.comment_col)
            exit(4)
        ...
        if r[uni_col] in students:
            ...
            if submit_grade:
                try:
                    ...
                    print('{0}:{1}: {2} '
                          .format(grades_name, i+2, r[grade_col]))
                    print('Not submitting grade for', r[uni_col])
                    print()
            if submit_comment:
                try:
                    if not type(r[comment_col]) == str:
                    ...
                    print(e)
                    print('{0}:{1}: {2} '
                          .format(grades_name, i+2, r[comment_col]))
    ...
    if not args.no_log:
        if (merrlen > 0 or gerrlen > 0 or cerrlen > 0):
            err_report = '{0} missing UNIs, {1} non-numeric grades, and ' \
                         '{2} invalid comments.\n'\
